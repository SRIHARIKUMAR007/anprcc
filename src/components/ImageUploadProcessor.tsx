...
const ImageUploadProcessor = () => {
  ...
  const { isConnected, isProcessing, processImage, logDetection } = useSupabaseBackend();
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      if (result.success && result.results && result.results.length > 0) {
        ...
        const success = await logDetection({
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 sm:p-4 bg-slate-800/30 rounded-lg border border-slate-700">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm sm:text-base text-white font-medium">Supabase ANPR Backend</span>
        </div>
        <Badge 
          variant="secondary"
          className="bg-green-500/20 text-green-400 border-green-500/30 px-2 py-1 text-xs sm:text-sm"
        >
      ...
      <Card className="bg-slate-800/50 border-slate-700">
        <CardHeader className="pb-3 sm:pb-6">
          <CardTitle className="text-base sm:text-lg text-white flex items-center">
            ...
            <Badge variant="secondary" className="ml-3 bg-green-500/20 text-green-400 border-green-500/30 text-xs">
              SUPABASE
            </Badge>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4 sm:space-y-6">
          ...
          >
            ...
            <div className="space-y-2 sm:space-y-4">
              {isProcessing ? (
                <Loader2 className="w-8 h-8 sm:w-12 sm:h-12 text-green-400 mx-auto animate-spin" />
              ) : (
                <Upload className="w-8 h-8 sm:w-12 sm:h-12 text-slate-400 mx-auto" />
              )}
              <div>
                <p className="text-sm sm:text-base text-white font-medium">
                  {isProcessing ? 'Processing with Supabase...' : 
                   isDragActive ? 'Drop the image here' : 
                   'Drag & drop an image here, or click to select'}
                </p>
                <p className="text-xs sm:text-sm text-slate-400 mt-1">
                  Supports: JPEG, PNG, BMP files • Powered by Supabase
                </p>
              </div>
            </div>
          </div>
                ...
                {isProcessing ? (
                  <div className="flex items-center justify-center h-48 sm:h-64 bg-slate-700/30 rounded-lg">
                    <div className="text-center space-y-2">
                      <Loader2 className="w-6 h-6 sm:w-8 sm:h-8 text-green-400 mx-auto animate-spin" />
                      <p className="text-xs sm:text-sm text-slate-400">
                        Processing with Supabase backend...
                      </p>
                    </div>
                  </div>
                ) : processingResult ? (
